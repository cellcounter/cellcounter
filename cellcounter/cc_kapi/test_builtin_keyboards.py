...
class KeyboardLayoutsTestCase(TestCase):
    ...
    def test_builtin_keyboard_factory(self):
        ...
        builtin_desktop_keyboard_factory = BuiltinKeyboardFactory(device_type=Keyboard.DESKTOP)
        builtin_mobile_keyboard_factory = BuiltinKeyboardFactory(device_type=Keyboard.MOBILE)
        [builtin_desktop_keyboard_marshall, builtin_mobile_keyboard_marshall] = KeyboardLayoutsMarshall().get_all(layout_types="builtin")
        self.assertEqual(builtin_desktop_keyboard_factory.serialize(), builtin_desktop_keyboard_marshall.serialize())
        self.assertEqual(builtin_mobile_keyboard_factory.serialize(), builtin_mobile_keyboard_marshall.serialize())
    ...
    def test_builtin_keyboards_count(self):
        ...
        builtin_keyboard_desktop = KeyboardLayoutsMarshall().get_all(layout_types="builtin", device=Keyboard.DESKTOP)
        self.assertEqual(len(builtin_keyboard_desktop), 1)
        self.assertEqual(builtin_keyboard_desktop[0].device(), Keyboard.DESKTOP)

        builtin_keyboard_mobile = KeyboardLayoutsMarshall().get_all(layout_types="builtin", device=Keyboard.MOBILE)
    ...
    def test_default_keyboards_count(self):
        builtin_keyboards = KeyboardLayoutsMarshall().get(device=Keyboard.DESKTOP, layout_id="default")
    ...
    def test_builtin_keyboards_fields_match(self):
        ...
        self.assertEqual(desktop_keyboard.serialize().keys(), builtin_desktop_keyboard.serialize().keys())
